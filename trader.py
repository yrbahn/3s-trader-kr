        ...
        for t in all_tickers:
            try:
                ...
                if not price_col.empty:
                    val = price_col.iloc[-1]; current_prices[t] = float(val.iloc[0]) if hasattr(val, 'iloc') else float(val)
            except: continue
    except: pass
    for entry in trajectory:
        ...
        for s in entry.get("selected", []):
            ...
            raw_code = s.get("stock_code"); match = re.search(r'(\d{6}\.K[SQ])', str(raw_code).upper()); code = match.group(1) if match else str(raw_code)
            buy_price = s.get("buy_price"); weight = s.get("weight", 1)
            if code in current_prices and buy_price and buy_price > 0:
                curr_p = current_prices[code]; ret = ((curr_p / buy_price) - 1) * 100
                s["current_price"] = int(curr_p); s["return"] = round(ret, 2); total_ret += ret * weight; total_weight += weight
...
def _get_stock_data(ticker: str) -> Dict[str, Any]:
    ...
    try:
        ...
        tech_summary = f"Price: {int(last_close)}, Weekly: {weekly_return}%, MACD: {macd_status}, VolSpike: {vol_spike}x, MA: {'Bullish' if safe_get(df['Close'].rolling(window=5).mean()) > safe_get(df['Close'].rolling(window=20).mean()) else 'Weak'}"
        ...
        return {"tech_text": tech_summary, "fund_text": f"Basic: PER:{_parse(soup_main, '_per')}, PBR:{_parse(soup_main, '_pbr')}, ROE:{roe}%, TargetPrice:{target_price}. DART:{dart_info}, Investor: F:{f_sum}, I:{i_sum}", "news_text": "\n".join(news_contexts), "price": int(last_close)}
    except: return {}
...
def main():
    print("3S-Trader KR: Perfect Multi-Agent Mode (DART+MACD+ROE Integrated)")
    today_str = datetime.now().strftime('%Y-%m-%d'); cache = load_cache(today_str)
    market_overview = get_market_overview(); trajectory = []
    ...
    universe = get_stock_universe(limit=30); to_do = [t for t in universe if t not in cache]
    ...
    def process(t):
        ...
        res = score_agent(t, n, f, te); scores = res.get('scores', {})
        print(f"[Anal: {t}] Scores: {scores}")
        return t, {"ticker": t, "name": stock.get_market_ticker_name(t.split('.')[0]), "scores": scores, "summaries": {"news": n, "technical": te, "fundamental": f}, "justifications": res.get("justifications", {}), "data": {"price": raw['price']}}
    ...
    sel_res = selection_agent(current_strategy, scored_sorted[:30]); final_stocks = sel_res.get("selected_stocks", [])
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        if past:
            ...
            for e in reversed(past):
                ...
                for s in e.get('selected', []):
                    c = s.get('stock_code', 'N/A'); match = re.search(r'(\d{6}\.K[SQ])', str(c))
        ...
        for s in final_stocks:
            code = str(s.get('stock_code','')); match = next((x for x in scored_universe if code in x['ticker'] or x['ticker'] in code), None)
